...
public:
  ...
  void normalize_weight_rows() {
    flush_ws(); 
    AzDmat m; m_w.get(&m); 
    AzDmat m_tran; m.transpose_to(&m_tran); 
    m_tran.normalize(); 
    m_tran.transpose_to(&m); 
    m_w.set(&m); 
    v_i.zeroOut(); 
  }
  /*--------------------------------*/
  ...
  virtual void updateDelta(int d_num, 
                           ...
                           const AzPmat *m_deriv) = 0; /* [class,data] */
  virtual void updateDelta(int d_num, 
                           const AzpLmParam &p, 
  ...
  virtual void initWeights(const AzpLmParam &p) {  
    ...
    AzRandGen rg; 
    if (p.do_iw_uniform) rg.uniform(p.initw_max, &md_w); 
...
public:
  ...
  virtual void updateDelta(int d_num, const AzpLmParam &p, const AzPmatSpa *m_x, const AzPmat *m_deriv) {
    untrainable("updateDelta(spa)"); 
  }
  virtual void updateDelta(int d_num, const AzpLmParam &p, const AzPmat *m_x, const AzPmat *m_deriv) {
