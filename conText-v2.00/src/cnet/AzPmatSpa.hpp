...
#if 0   
  void set(const AzSmatVar *mv0, int d0_begin, int d0_end, bool do_gen_rowindex); /* this[,0::d0_end-d0_begin] <- mv0[,d0_begin:d0_end-1] */
#endif   
  template <class M> /* M: AzSmatVar | AzSmatcVar */
...
public: 
  ...
  static void elm_multi(AzPmat *md, const AzPmatSpa *ms) { 
    AzPmat m(ms->rowNum(), ms->colNum()); ms->add_to(&m, 1); 
    md->elm_multi(&m); 
  }
  static void elm_multi(AzPmat *m0, const AzPmat *m1)    { m0->elm_multi(m1); }  
