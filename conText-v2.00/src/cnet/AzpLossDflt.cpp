                          ...
                          AzBytArr *s_pf) const
{
  ...
  if (is_regression) {
    ...
    if (do_force_eval) {
      ...
      Az_PRF prf = AzpEv::eval_micro(m_p, m_y, th); 
      AzBytArr s; s << "precision=" << prf.prec << ",recall=" << prf.recall;  
      AzPrint::writeln(out, s); 
      if (s_pf != NULL) s_pf->reset("1-F"); 
      return 1-prf.fval; 
    }
    else {
      if (s_pf != NULL) s_pf->reset("NO-SUPPORT"); 
      return -1;       
    }
  }
  /*---  classification  ---*/
...
double AzpLossDflt::_test_eval(int data_size, 
                          ...
                          AzBytArr *s_pf) const
{
  const char *eyec = "AzpLossDflt::_test_eval_regress(sparse)"; 
