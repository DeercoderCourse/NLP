                    ...
                    bool do_gen_row_index)
{
  ...
  AzX::throw_if((ms == NULL || cxs == NULL), eyec, "Null input"); 
  ...
  for (int col = 0; col < col_num; ++col) {
    ia_ptrs.put(index); /* beginning of this column */
    int elm_num = 0; 
    const AZI_VECT_ELM *elm = ms->rawcol(cxs[col], &elm_num); 
    for (int ix = 0; ix < elm_num; ++ix) {
      hvals[index] = elm[ix].val; 
      ia_rows(index, elm[ix].no); 
      ia_cols(index, col);      
      if (do_gen_row_index && !f.do_cu_x) {
        iifa.put(elm[ix].no, col, elm[ix].val); 
      }
      ++index;       
    }
  }
...
/*------------------------------------------------*/
template <class M> /* M: AzSmatcVar | AzSmatVar */
