...
protected: 
  ...
  clock_t curr[AzpTimer_CNN_ClockMax];
  void reset() {
    clock_t clk = AzPdevice::sync_time(); 
    for (int ix = 0; ix < AzpTimer_CNN_ClockMax; ++ix) curr[ix] = clk; 
  }
  void stamp(int clock_no=0, clock_t *period=NULL) {
    AzX::throw_if((clock_no < 0 || clock_no >= AzpTimer_CNN_ClockMax), "AzpTimer_CNN::stamp", "invalid clock#"); 
    clock_t prev = curr[clock_no]; curr[clock_no] = AzPdevice::sync_time(); 
  ...
  int layer_num; 
  #define _maxtyp_ 25
  clock_t total[_maxtyp_]; 
  #define _maxlay_ 50
  clock_t lay[_maxtyp_][_maxlay_]; 
  
public:
  AzpTimer_CNN() : layer_num(0) {
    reset(1); /* dummy */
  }
...
protected:   
  clock_t format(AzBytArr &s, const char *nm, clock_t ticks) const {
