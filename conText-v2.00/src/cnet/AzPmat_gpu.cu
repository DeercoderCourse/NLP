...
void _AzParr<T>::free() {
  if (elm != NULL) {  
    dev.pmem.free(elm, sizeof(T)*num);      
    elm = NULL; 
  }
  num = 0; 
}  
...
void _AzParr<T>::free_alloc(int inp_num, const char *str1, const char *str2) {
  ...
  if (inp_num > 0) {
    ...
    elm = (T *)dev.pmem.alloc(sz, str1, str2);   
    num = inp_num; 
  }
  else if (inp_num < 0) {
...
{
  const char *eyec = "_AzPmat::_squareSum_cublas"; 
  AzFloat nrm2 = 0; 
...
{
  AzX::throw_if((!is_rg_set), "_AzPrng::uniform", "not ready"); 
