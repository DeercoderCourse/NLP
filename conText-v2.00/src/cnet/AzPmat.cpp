...
void AzPmat::add_s2d(const AzPmat *m, const int *cols, int cnum, double coeff) 
{
  ...
  AzPintArr pia_cols; 
  pia_cols.reset(cols, cnum); 
  u._add_cols_s2d(_dptr_u(), m->_dptr(), row_num, pia_cols._dptr(), pia_cols.size(), (AzFloat)coeff); 
}
...
/*------------------------------------------------*/
void AzPmat::multiply_eachcol(const AzPmat *m2, bool do_inv)
{
  const char *eyec = "AzPmat::multiply_eachcol(m2)"; 
  AzX::throw_if((col_num != m2->col_num || m2->rowNum() != 1), eyec, "the coefficient matrix has a wrong shape."); 
...
void AzPmat::colSum(const AzPmat *m_inp, bool do_chk) /* this <- colSum(inp) */
{
  ...
  reform_chk(r_num, c_num, do_chk, "AzPmat::colSum"); 
...
void AzPmat::rowSum(const AzPmat *m_inp, bool do_chk) /* this <- rowSum(inp) */
{
  ...
  int c_num = 1; 
  reform_chk(r_num, c_num, do_chk, "AzPmat::rowSum"); 
...
void AzPmat::add_colSum(const AzPmat *m_inp) /* this += colSum(inp) */
{
  int r_num = 1; 
  int c_num = m_inp->col_num; 
  shape_chk(r_num, c_num, "AzPmat::add_colSum"); 
...
void AzPmat::add_rowSum(const AzPmat *m_inp) /* this += rowSum(inp) */
{
  int r_num = m_inp->row_num; 
  int c_num = 1; 
  shape_chk(r_num, c_num, "AzPmat::add_rowSum"); 
