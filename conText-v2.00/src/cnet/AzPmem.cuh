...
public:   
  AzPmem() : mem(NULL), memsz(0), curr_extra(0), extra_count(0), curr_mem(0), max_mem(0) {}
  ~AzPmem() { term(); }
  ...
  void init(size_t _memsz) { 
    ...
    AzX::throw_if((mem != NULL), eyec, "mem must be NULL at this point"); 
    ent.reset(); 
    memsz = MAX(_memsz, 0); 
    if (memsz <= 0) return; 
  ...
  /*--------------------------------------------------------------------------------*/
  void *alloc(size_t _sz, const char *str1="", const char *str2="") {
    ...
    for ( ; no >= 0; ) {
      ...
      AzX::throw_if((ep->ptr == NULL), eyec, "null ptr?!");  
  ...
  /*--------------------------------------------------------------------------------*/
  void free(void *ptr, size_t _sz, const char *str1="") {
    ...
    for ( ; no >= 0; ) {
      const AzPmemEnt *ep = ent[no]; 
      if (ep->ptr == ptr) {
        AzX::throw_if((sz != ep->sz), eyec, "size mismatch.  something is wrong ... "); 
    ...
    AzX::throw_if((curr_extra <= 0), eyec, "extra: # of free requests is greater than # of alloc requests ... "); 
    --curr_extra; 
...
protected:   
  ...
  int insert_after(int no) {
    ...
    AzX::throw_if((ex >= ent.size()), "AzPmem::insert_after", "Too many concurrent use of device memory areas"); 
  ...
  void merge_next_if_possible(int no) {
    ...
    AzX::throw_if((ent[no]->ptr + ent[no]->sz != ent[next]->ptr), eyec, "The two areas should be adjacent ... ?!"); 
  ...
  void check_consistency() const {
    ...
    size_t sz = 0; 
    for ( ; no >= 0; ) {
      AzX::throw_if((ent[no]->prev != prev), eyec, "wrong prev"); 
      AzX::throw_if((ent[no]->ptr == NULL), eyec, "null ptr"); 
      AzX::throw_if((prev >= 0 && ent[prev]->ptr+ent[prev]->sz != ent[no]->ptr), eyec, "areas are not adjacnet"); 
      sz += ent[no]->sz; 
      prev = no; no = ent[no]->next; 
    }
    AzX::throw_if((sz != memsz), eyec, "total memory size doesn't match"); 
  }
}; 
#endif 