...
protected:
  ...
  /*---  Exponential: eta MAX(b, pow(b^{1/a}, t)) ... reaches eta*b after a epochs ---*/
...
public: 
  ...
  #define kw_typ "step_size_scheduler="
  #define kw_decay_factor "step_size_decay="
  #define kw_decay_at "step_size_decay_at="
  ...
  void resetParam(AzParam &azp, const char *pfx="") {
    ...
    azp.vStr(kw_typ, &s_typ);     
    if (s_typ.length() > 0) {
      AzStrPool sp_supported_typ(10,10); 
      sp_supported_typ.put("Tinv", "Expo", "Linear", "Few", "None"); 
    ...
    if (typ == AzStepszSch_Type_Few) {
      azp.vStr(kw_aa, &s_decay_at); /* for compatibility */
      azp.vFloat(kw_bb, &decay_factor); /* for compatibility */    
    
      azp.vStr(kw_decay_at, &s_decay_at); 
      azp.vFloat(kw_decay_factor, &decay_factor);
    ...
    else if (typ == AzStepszSch_Type_Expo || typ == AzStepszSch_Type_Linear) {
      /*---  Exponential: eta MAX(b, pow(b^{1/a}, t)) ... reaches eta*b after a epochs ---*/
      /*---  Linear: eta MAX(b, 1 - (1-b)/a t) ... reaches eta*b after a epochs  ---*/
      azp.vFloat(kw_aa, &aa); 
      azp.vFloat(kw_bb, &bb); 
  ...
  void printParam(const AzOut &out, const char *pfx="") const {
    AzPrint o(out, pfx); 
    o.printV_if_not_empty(kw_typ, s_typ); 
    o.printV(kw_decay_factor, decay_factor); 
    o.printV_if_not_empty(kw_decay_at, s_decay_at); 
...
/*    h.item(kw_typ, "Step-size scheduler type.  \"Few\" (reduce it a few times) | \"Linear\" | \"Expo\" (exponential)"); */
    h.item(kw_typ, "Step-size scheduler type.  \"Few\" (reduce it a few times). ", "no scheduling");
...
/*    h.item(kw_cc, "C: Use this with \"Tinv\".  (1+(B^{-1/C}-1)/A t)^{-C}"); */
  ...
  double get_stepsize_coeff(int tt) {
    ...
    if (typ == AzStepszSch_Type_Few) {
      ...
      /*---  Exponential: eta pow(b^{1/a}, t) ... reaches eta*b after a epochs ---*/  
