...
public:
  AzDic() {}
  AzDic(const char *fn, bool do_ignore_dup=false) {  
    reset(fn, do_ignore_dup); 
  }
  AzDic(const AzBytArr *s_fn, bool do_ignore_dup=false) {
    reset(s_fn->c_str(), do_ignore_dup); 
  }
  ...
  void readText(const char *fn, bool do_ignore_dup=false) { reset(fn, do_ignore_dup); }
  void reset(const char *fn, bool do_ignore_dup=false) {  
    sp_words.reset(); sp_sorted.reset(); 
    sp_words.reset(100000, 10);   
    AzByte dlm = '\t'; 
    bool do_count = true; 
    AzTools::readList(fn, &sp_words, &dlm, do_count);
  ...
  int find_ngram(const AzStrPool *sp_words, int wx, /* position in sp_words */
                 int nn) const {
    if (wx < 0 || wx+nn > sp_words->size() || size() <= 0) {
      return -1; 
    }
    AzBytArr s;   
    AzTools::compose_ngram(sp_words, wx, nn, &s); 
    return find(s.c_str()); 
  } 
