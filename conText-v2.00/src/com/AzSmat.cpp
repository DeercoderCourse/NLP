...
void AzSmat::initialize(int inp_row_num, int inp_col_num, bool asDense) 
{
  ...
  if (asDense) {
    for (int cx = 0; cx < col_num; ++cx) {
      column[cx] = AzSvect::new_svect(this->row_num, asDense); 
    }
  }
...
void AzSmat::set(const AzSmat *inp)  
{
  ...
  for (int cx = 0; cx < this->col_num; ++cx) {
    if (inp->column[cx] == NULL) {
      ...
      if (column[cx] == NULL) {
        column[cx] = AzSvect::new_svect(row_num); 
      }
  ...
  for (int cx = col0; cx < col1; ++cx) {
    ...
    if (inp->column[cx] == NULL) {
      ...
      if (column[my_cx] == NULL) {
        column[my_cx] = AzSvect::new_svect(row_num); 
      }
  ...
  for (my_col = 0; my_col < cnum; ++my_col) {
    ...
    if (inp->column[col] == NULL) {
      ...
      if (column[my_col] == NULL) {
        column[my_col] = AzSvect::new_svect(row_num); 
      }
...
void AzSmat::set(int col0, int col1, const AzSmat *inp, int i_col0)
{
  ...
  for (int col = col0; col < col1; ++col, ++i_col) {
    if (inp->column[i_col] == NULL) {
      ...
      if (column[col] == NULL) {
        column[col] = AzSvect::new_svect(row_num); 
      }
  ...
  for (new_col = 0; new_col < cnum; ++new_col) {
    ...
    if (old_col == new_col) {}
      ...
      if (column[new_col] == NULL) {
        column[new_col] = AzSvect::new_svect(row_num); 
      }
                           ...
                           bool do_zero_negaindex)
{
  ...
  for (ex = 0; ex < elm_num; ++ex) {
    ...
    if (ex > 0 && elm[ex].no <= elm[ex-1].no) {
      throw new AzException(eyec, "rows are out of order.");  /* added feb02_2015 */
    }
  }
  row_num = new_row_num; 
}
...
AZint8 AzSmat::nonZeroNum(double *ratio) const
{
  ...
  for (int cx = 0; cx < col_num; ++cx) {
    if (column[cx] != NULL) out += column[cx]->nonZeroRowNum(); 
  }
  if (ratio != NULL && out != 0) {
...
void AzSmat::_read(AzFile *file) 
{
  ...
  if (marker == -1) { /* new version after 12/14/2014 */
    ...
    throw new AzException(AzInputError, eyec, "Unknown marker was detected.  The file is not AzSmat."); 
  }
...
/*-------------------------------------------------------------*/
void AzSmat::_write_cols(AzFile *file)
{
...
/*-------------------------------------------------------------*/
void AzSvect::_write(AzFile *file, int rnum) 
{
  if (rnum >= 0) {
    /* save space by not writing #row; caller should be AzSmat */
    if (row_num != rnum) throw new AzException("AzSvect::write", "#row conflict!"); 
  }
  else {
    file->writeInt(row_num); 
  }
...
void AzSvect::check_rowno() const {
  for (int ex = 0; ex < elm_num; ++ex) {
    if (elm[ex].no < 0 || elm[ex].no >= row_num) {
      ...
      throw new AzException("AzSvect::check_rowno", "row# is out of range. ", s.c_str()); 
    }
  }
}
...
void AzSvect::_read(AzFile *file, int float_size, int rnum) 
{
  ...
  if (float_size == -1 || float_size == sizeof(AZ_MTX_FLOAT)) {
    ...
    if (isSwapNeeded) {
      for (int ex = 0; ex < elm_num; ++ex) {
        AzFile::swap_int4(&elm4[ex].no); 
        AzFile::swap_float(&elm4[ex].val); 
      }
    }
    for (int ex = 0; ex < elm_num; ++ex) {
    ...
    if (isSwapNeeded) {
      for (int ex = 0; ex < elm_num; ++ex) {
        AzFile::swap_int4(&elm8[ex].no); 
        AzFile::swap_double(&elm8[ex].val); 
      }
    }
    for (int ex = 0; ex < elm_num; ++ex) {
    ...
    throw new AzException(eyec, "Unexpected floating-point variable size."); 
  }
...
void AzSvect::rawset(const AZI_VECT_ELM *ielm, int ielm_num)
{
  ...
  if (ielm_num < 0) throw new AzException(eyec, "ielm_num<0"); 
  a.free(&elm); 
  elm_num = ielm_num; 
  if (elm_num <= 0) return; 
  if (ielm == NULL) throw new AzException(eyec, "null input"); 
  for (int ex = 0; ex < ielm_num; ++ex) {
    if (ielm[ex].no < 0 || ielm[ex].no >= row_num) throw new AzException(eyec, "row# is out of range"); 
    if (ex > 0 && ielm[ex].no <= ielm[ex-1].no) throw new AzException(eyec, "elements are not sorted"); 
  }  
...
void AzSmatc::set(const AzSmatc *m)
{
  ...
  if (m == NULL) throw new AzException(eyec, "null input"); 
  reform(m->rowNum(), m->colNum()); 
  int elm_num = (int)m->elmNum(); 
  arr.alloc(&elm, elm_num, eyec); 
  ia_begin.reset(&m->ia_begin); 
  ia_end.reset(&m->ia_end); 
...
void AzSmatc::set(const AzSmatc *m, const int *cxs, int cxs_num)
{
  ...
  int ex = 0; 
  int new_col; 
  for (new_col = 0; new_col < cxs_num; ++new_col) {
    const AZI_VECT_ELM *inp = m->rawcol(cxs[new_col]); 
    int inp_num = m->col_size(cxs[new_col]); 
    if (ex+inp_num > elm_num) throw new AzException(eyec, "something is wrong 1"); 
    ia_begin(new_col, ex); 
    memcpy(elm+ex, inp, inp_num*sizeof(elm[0])); 
    ex += inp_num; 
    ia_end(new_col, ex); 
  }
  if (ex != elm_num) throw new AzException(eyec, "something is wrong 2"); 
  if (new_col != col_num) throw new AzException(eyec, "something is wrong 3"); 
}  
...
void AzSmatc::set(const AzSmat *m)
{
  ...
  arr.alloc(&elm, elm_num, eyec); 
  ia_begin.reset(col_num, -1); 
  ia_end.reset(col_num, -1); 
  int ex = 0; 
  for (int cx = 0; cx < m->colNum(); ++cx) {
    ia_begin(cx, ex); 
    AzIFarr ifa; m->col(cx)->nonZero(&ifa); 
    for (int ix = 0; ix < ifa.size(); ++ix, ++ex) {
      if (ex >= elm_num) throw new AzException(eyec, "something is wrong 1"); 
      elm[ex].no = ifa.getInt(ix); 
      elm[ex].val = (AZ_MTX_FLOAT)ifa.get(ix); 
    }
    ia_end(cx, ex); 
  }
  if (ex != elm_num) throw new AzException(eyec, "something is wrong 2"); 
...
/*-------------------------------------------------------------*/
void AzSmatc::cbind(const AzSmatc *m)
{
  const char *eyec = "AzSmatc::cbind"; 
  if (m == NULL) return; 
  if (m->row_num <= 0 || m->col_num <= 0) return; 
  if (row_num <= 0 || col_num <= 0) {
    set(m); 
    return;     
  }
  if (row_num != m->row_num) throw new AzException(eyec, "#row mismatch"); 
  
  int elm_num = arr.size(), elm_num1 = m->arr.size(); 
  int new_elm_num = Az64::to_int((size_t)elm_num+(size_t)elm_num1, eyec); 
  
  AzIntArr ia_begin1(&m->ia_begin); ia_begin1.add(elm_num); 
  AzIntArr ia_end1(&m->ia_end); ia_end1.add(elm_num); 
  
  arr.realloc(&elm, new_elm_num, eyec); 
  memcpy(elm+elm_num, m->elm, sizeof(elm[0])*elm_num1); 
  ia_begin.concat(&ia_begin1); 
  ia_end.concat(&ia_end1); 
  col_num += m->colNum(); 
}

/*------------------------------------------------*/
/* called by AzMatVar::transfer_from */
void AzSmat::transfer_from(AzDataArr<AzSmat> *am, const AzIntArr &ia_dcolind, 
                            int r_num, int c_num, AZint8 e_num) 
{  
  const char *eyec = "AzSmat::transfer_from"; 
  int data_num = ia_dcolind.size()/2; 
  reform(r_num, c_num); 
  for (int dx = 0; dx < data_num; ++dx) {
    AzSmat *m_inp = am->point_u(dx); 
    int col0 = ia_dcolind[dx*2], col1 = ia_dcolind[dx*2+1]; 
    if (m_inp->colNum() != col1-col0) throw new AzException(eyec, "conflict btw matrix array and index"); 
    for (int col = col0; col < col1; ++col) col_u(col)->set(m_inp->col(col-col0)); 
    m_inp->destroy(); 
  }
}
...
/*------------------------------------------------*/
/* called by AzMatVar::transfer_from */
void AzSmatc::transfer_from(AzDataArr<AzSmat> *am, const AzIntArr &ia_dcolind, 
                            int r_num, int c_num, AZint8 e_num) 
{  
  const char *eyec = "AzSmatc::transfer_from"; 
  int data_num = ia_dcolind.size()/2; 
  reform(r_num, c_num); 
  int elm_num = Az64::to_int((size_t)e_num, eyec); 
  arr.alloc(&elm, elm_num, eyec); 
  int ex = 0; 
  for (int dx = 0; dx < data_num; ++dx) {
    AzSmat *m_inp = am->point_u(dx); 
    ...
    for (int col = col0; col < col1; ++col) {
      ia_begin(col, ex); 
      const AzSvect *v = m_inp->col(col-col0); 
      for (int ix = 0; ix < v->elm_num; ++ix) {
        if (v->elm[ix].val != 0) elm[ex++] = v->elm[ix];       
      }
      ia_end(col, ex); 
    }  
    m_inp->destroy(); 
  ...
  if (ex != elm_num) {
    throw new AzException(eyec, "something is wrong ..."); 
  }  
}

/*-------------------------------------------------------------*/
void AzSmatc::write(AzFile *file) 
{
  ...
  for (int cx = 0; cx < col_num; ++cx) {
    int num = col_size(cx); 
    file->writeInt(num); 
    file->writeBytes(rawcol(cx), num*sizeof(elm[0])); 
  }
  _swap(); 
}
...
void AzSmatc::read(AzFile *file) 
{
  ...
  int marker = file->readInt();
  if (marker != -1) {
    throw new AzException(AzInputError, eyec, "Unknown marker was detected.  The file is not AzSmat or old AzSmat before 12/14/2014."); 
  }
  float_size = file->readInt();
  col_num = file->readInt();
  row_num = file->readInt();
...
void AzSmatc::check_rowno(int where, int elm_num) const {
  ...
  for (int ex = where; ex < where+elm_num; ++ex) {
    if (elm[ex].no < 0 || elm[ex].no >= row_num) {
      AzBytArr s; s << "#row=" << row_num << ", elm[ex].no=" << elm[ex].no << ", ex=" << ex; 
      throw new AzException(eyec, "row# is out of range. ", s.c_str()); 
    }
    if (ex > where && elm[ex].no <= elm[ex-1].no) {
      AzBytArr s; s << "ex=" << ex << " elm[ex-1].no=" << elm[ex-1].no << " elm[ex].no=" << elm[ex].no; 
      throw new AzException(eyec, "row# is out of order. ", s.c_str()); 
    }
  }
}
...
void AzSmatc::copy_to_smat(AzSmat *ms, const int *cxs, int cxs_num) const
{
  ...
  for (int ix = 0; ix < cxs_num; ++ix) {
    int cx = cxs[ix]; 
    ms->col_u(ix)->rawset(rawcol(cx), col_size(cx)); 
  }
}
