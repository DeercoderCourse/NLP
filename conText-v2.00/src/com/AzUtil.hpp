...
#define AzLittleEndian_isSwapNeeded false
  ...
  /**
   *  Endian-insensitive applications such as RGF should call this.  
  ...
  /**
   *  Endian-sensitive applications such as ConText should call this. 
   */
  void Az_check_system2_() {
    if (sizeof(int) != 4 || sizeof(double) != 8 || sizeof(bool) != 1 || sizeof(AZint8) != 8) {
      throw new AzException("_check_system2_", 
            "expecting sizeof(int)=4,sizeof(bool)=1,sizeof(double)=8,sizeof(AZint8)=8");
    }
  }  
  /*---  end of main  ---*/
...
private: 
  ...
  void check_fp(const char *eyec) const {
    if (fp == NULL) throw new AzException(eyec, "Open the file before this operation."); 
  }
...
public: 
  ...
  AZint8 writeBytes(const void *buff, AZint8 len) {
    ...
    if (len == 0) return 0; 
    check_fp(eyec); 
    if (fwrite(buff, (size_t)len, 1, fp) != 1) {
  ...
  AZint8 writeBytes(const void *buff, AZint8 sz, AZint8 count) {
    ...
    check_fp(eyec);     
    if (fwrite(buff, (size_t)sz, (size_t)count, fp) != count) {
      throw new AzException(AzFileIOError, eyec, pointFileName(), "fwrite");
    }
    return sz*count; 
  } 
  ...
  inline static void swap_float(float *x) {
    if (!isSwapNeeded) return; 
    _swap((AzByte *)x, 0,3); 
    _swap((AzByte *)x, 1,2); 
  }  
...
public:
  ...
  bool isAlpha() const {
    ...
    int len = length(); 
    int ix; 
    for (ix = 0; ix < len; ++ix) if (!isalpha(bytes[ix])) return false; 
    return true; 
  }
...
public:
  ...
  void writec(AzFile *file) const { /* 12/07/2014 */
    AzIntArr ia(this); 
    ia.write(file); 
  }
...
public:
  ...
  AzIIFarr(int num) : ent_num(0), ent(NULL), ent_type(AzIIFarr_IIF) {
    prepare(num); 
  }
  ~AzIIFarr() {
