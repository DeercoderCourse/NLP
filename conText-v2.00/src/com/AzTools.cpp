...
#if 0 
        count = atol((char *)(str+ix+1)); /* +1 for dlm */
#else   
...
/*------------------------------------------------*/
void AzTools::writeMatrix(AzDmat *m, const char *fn, const AzOut &out, int digits)
{
  if (m == NULL) throw new AzException("AzTools::writeMatrix(dmat)", "null input"); 
...
/*------------------------------------------------*/
void AzTools::writeMatrix(AzSmat *m, const char *fn, const AzOut &out, int digits)
{
  if (m == NULL) throw new AzException("AzTools::writeMatrix(smat)", "null input"); 
  AzBytArr s("Writing ", fn, ": "); show_smat_stat(*m, s); 
  if (AzBytArr::endsWith(fn, "dmat")) {  
    AzDmat md(m); 
    AzTimeLog::print(s.c_str(), " (dmat)", out); 
    md.write(fn); 
  }
  else if (AzBytArr::endsWith(fn, "smat")) {
    AzTimeLog::print(s.c_str(), " (smat)", out); 
    m->write(fn); 
  }
  else {
    AzTimeLog::print(s.c_str(), " (text)", out); 
    bool do_sparse = true; 
    m->writeText(fn, digits, do_sparse); 
  }
  AzTimeLog::print("Done ... ", out); 
}

/*------------------------------------------------*/
bool AzTools::readMatrix(const char *fn, AzDmat *m, const AzOut &out)
{
  if (m == NULL) new AzException("AzTools::readMatrix(dense)", "null output"); 
  if (AzBytArr::endsWith(fn, "dmat")) {
    AzTimeLog::print("Reading dmat: ", fn, out); 
    m->read(fn); 
    AzTimeLog::print("Done ... ", out); 
    return true; 
  }
  else if (AzBytArr::endsWith(fn, "smat")) {
    AzTimeLog::print("Reading smat: ", fn, out); 
    AzSmat ms; 
    ms.read(fn); 
    AzTimeLog::print("Converting to dmat ... ", out); 
    m->set(&ms); 
    AzTimeLog::print("Done ... ", out); 
    return true; 
  }
...
/*------------------------------------------------*/
bool AzTools::readMatrix(const char *fn, AzSmat *m, const AzOut &out)
{
  if (m == NULL) new AzException("AzTools::readMatrix(sparse)", "null output"); 
  if (AzBytArr::endsWith(fn, "dmat")) {
    AzTimeLog::print("Reading dmat: ", fn, out); 
    AzDmat md; 
    md.read(fn); 
    AzTimeLog::print("Converting to smat ... ", out); 
    md.convert(m); 
    AzTimeLog::print("Done ... ", out); 
    return true; 
  }
