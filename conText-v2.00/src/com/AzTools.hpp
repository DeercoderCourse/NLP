...
public: 
  ...
  static void writeMatrix(AzDmat *m, const char *fn, const AzOut &out, int digits=7); 
  static void writeMatrix(AzSmat *m, const char *fn, const AzOut &out, int digits=7); 
  static bool readMatrix(const char *fn, AzDmat *m, const AzOut &out); 
  static bool readMatrix(const char *fn, AzSmat *m, const AzOut &out); 
  ...
  inline static void compose_ngram(const AzStrPool *sp_words, 
    ...
    for (ix = wx; ix < wx+n; ++ix) {
      ...
      if (sp_stop != NULL && sp_stop->find(sp_words->c_str(ix)) >= 0 || 
          do_remove_number && strpbrk(sp_words->c_str(ix), "0123456789") != NULL) {
        s_ngram->reset(); 
        return; 
      }
      if (do_n_remove_nonalpha) {
        AzBytArr s_word; sp_words->get(ix, &s_word); 
        if (!s_word.isAlpha()) {
          s_ngram->reset(); 
          return; 
        }
  ...
  template <class M> static void write_mat(const char *fn, M *m) {
    AzBytArr s("Writing ", fn, ": "); show_dmat_stat(*m, s); 
...
protected: 
  ...
  AzFile file; 
public: 
  AzMats_file() : num(0), cur(0), mode(0) {}
  AzMats_file(const char *fn) : num(0), cur(0), mode(0) {
    reset_for_read(fn); 
  }
  AzMats_file(const char *fn, int num) : num(0), cur(0), mode(0) {
    reset_for_write(fn, num); 
  }
  ...
  void write(M *mat) {
    ...
    if (mode != 'w') throw new AzException(eyec, "call reset_for_write first"); 
    if (mat == NULL) throw new AzException(eyec, "null input"); 
    if (cur >= num) throw new AzException(eyec, "index is out of range"); 
    mat->write(&file); 
    ++cur; 
