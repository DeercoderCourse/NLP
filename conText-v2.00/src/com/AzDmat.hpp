  ...
  void mysqrt(double eps=0) {
    ...
    for (ix = 0; ix < num; ++ix) {
      if (elm[ix]<0) {
        ...
        throw new AzException("AzDvect::mysqrt", "Negative component"); 
      }
      elm[ix]=sqrt(elm[ix]); 
    }
  }
...
public: 
  ...
  void add(const AzDvect *v, double coeff=1) { /* add the vector to each column */
    if (v->rowNum() != row_num) {
      throw new AzException("AzDmat::add(dvect)", "#row mismatch"); 
    }
  ...
  /*---  x times transpose(x)  ---*/
  void add_xxT(const AzDvect *v, double coeff=1) {
    ...
    for (rx = 0; rx < v->rowNum(); ++rx) {
      col_u(rx)->add(v, val[rx]*coeff); 
    }
  }
  
  void xxT(const AzDmat *m, const AzDvect *v_coeff=NULL) {
    if (v_coeff != NULL && m->colNum() != v_coeff->rowNum()) {
      throw new AzException("AzDmat::xxT", "coefficient dimensionality mismatch"); 
    }
    reform(m->rowNum(), m->rowNum()); 
    int cx;
    for (cx = 0; cx < m->colNum(); ++cx) {
      double coeff = (v_coeff == NULL) ? 1 : v_coeff->get(cx); 
      add_xxT(m->col(cx), coeff); 
    }
  }
...
public:
  ...
  AzDmatc(const char *fn) : row_num(0), col_num(0), elm(NULL) {
    read(fn);  
  }
  void reset() {
  ...
  void read(AzFile *file) {
    ...
    reset(); 
    int float_size = file->readInt(); 
    if (float_size != sizeof(elm[0])) {
      AzBytArr s("Float size mismatch.  Expected "); s << (int)sizeof(elm[0]) << "; actual is " << float_size; 
  ...
  void writeText(const char *fn, int digits) {
    AzFile file(fn); 
    file.open("wb"); 
    writeText(&file, digits); 
    file.close(true); 
  }
  void writeText(AzFile *file, int digits) {
...
protected:
  void check_col(int col, const char *msg="") const {
    if (col < 0 || col >= col_num) throw new AzException("AzDmatc::check_col", msg); 
  }  
}; 
