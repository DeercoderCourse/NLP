...
protected:
  ...
  AzBytArr s; /* work */
public:
  inline AzPrint(const AzOut &out, const char *_pfx="") : o(NULL), dlm(NULL), name_dlm(NULL), did_write(false), 
  ...
  inline void reset(const AzOut &out, const char *_pfx="") {
    o = NULL; 
    if (!out.isNull()) {
      o = out.o; 
    }
  ...
  inline void printSw(const char *_kw, bool val) {
    if (o == NULL) return; 
  ...
  template<class T>
  inline void printV(const char *_kw, T val) {
    if (o == NULL) return; 
    const char *kw = cpfx(_kw);     
    if (val != -1) {
      itemBegin();  
  ...
  template<class T>
  inline void printV_posiOnly(const char *_kw, T val) {
    if (o == NULL) return; 
    const char *kw = cpfx(_kw);     
    if (val > 0) {
      itemBegin();  
  ...
  inline void printV(const char *_kw, const char *val) {
    if (o == NULL) return; 
    const char *kw = cpfx(_kw);     
    itemBegin(); 
    *o<<kw<<val; 
  }
  inline void printV(const char *_kw, const AzBytArr &s) {
    if (o == NULL) return; 
    const char *kw = cpfx(_kw);     
    itemBegin(); 
    *o<<kw<<s.c_str(); 
  }
  inline void printV_if_not_empty(const char *_kw, const AzBytArr &s) {
    if (o == NULL) return; 
