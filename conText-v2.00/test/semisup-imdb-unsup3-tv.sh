  #####
  #####  IMDB: unsup3-tv.  tv-embedding learning with unsupervised target and region vectors of bag-of-1-3grams. 
  #####
  ...
  uns_lst=data/imdb${z}-trnuns.lst
  ...
  logfn=log_output/imdb-unsx3-dim${dim}.log
  echo 
  ...
  #***  Step 2. Supervised training using tv-embedding to produce additional input. 
  #---  Step 2.1. Generate input files.
  xvoc1=data/imdb${z}-trn-${xszk}k.vocab
  $prep gen_vocab input_fn=data/imdb-train.txt.tok vocab_fn=$xvoc1 $options WriteCount max_vocab_size=${xszk}000
  ...
  logfn=log_output/imdb-unsx3-sup-dim${dim}.log
  perffn=perf/imdb-unsx3-sup-dim${dim}.csv
  ...
  $cnet $gpumem cnn 0side0_fn=$s_fn \
        trnname=imdb${z}-train-p${pch_sz} tstname=imdb${z}-test-p${pch_sz} data_ext0=seq data_ext1=x3bow \
