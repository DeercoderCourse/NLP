...
#  sdir=output    # <=   output/ if generated by semisup-imdb-{unsup|unsup3|parsup}-tv.sh.  
  ...
  #---  Step 1. Generate input files. 
  xszk=30
  xvoc1=data/imdb${z}-trn.vocab  # This must be the same as semisup-imdb-{unsup|unsup3|parsup}-tv.sh.
  $prep gen_vocab input_fn=data/imdb-train.txt.tok vocab_fn=$xvoc1 $options WriteCount \
  ...
  xvoc3=data/imdb${z}-trn-123gram-${xszk}k.vocab  # This must be the same as semisup-imdb-unsup3-tv.sh 
  echo Merging and sorting 1-3grams ... 
  perl merge_dic.pl ${xszk}000 $fns > $xvoc3
