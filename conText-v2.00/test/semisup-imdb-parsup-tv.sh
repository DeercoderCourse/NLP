  #####
  #####  IMDB: parsup-tv.  tv-embedding learning with partially-supervised target. 
  #####
  ...
  mod_fn=for-parsup-imdb-p${f_pch_sz}.supmod.ite100   # input: model trained by train_imdb_seq.sh 
  if [ ! -e $mod_fn ]; then
    wget riejohnson.com/software/${mod_fn}.gz # <= Download the model, or do training as in train_imdb_seq.sh, save the model and rename it. 
    gzip -d -f ${mod_fn}.gz                     
  fi
  ...
  rnm=data/imdb${z}-uns-p${f_pch_sz}
  ...
  dist=$pch_sz
  nm=imdb-parsup-p${f_pch_sz}p${pch_sz}
  ...
  logfn=log_output/imdb-parsup-dim${dim}.log
  echo 
  ...
  logfn=log_output/imdb-parsup-sup-dim${dim}.log
  perffn=perf/imdb-parsup-sup-dim${dim}.csv
