...
def extract_features(instance):
    ...
    # add new features to enhance the original one
    feature_set["length"]=len(text)
    
    # split the text by blank space to form a list, find most frequent word and its count
    list_words = text.split(" ")
    feature_set["mostfrequentword"]= str(collections.Counter(list_words).most_common(1)[0][0])
    feature_set["mostcount"]=str(collections.Counter(list_words).most_common(1)[0][1])

    # other useful feature
    feature_set["middleword"]=text[int(len(text)/2)]
    return feature_set
