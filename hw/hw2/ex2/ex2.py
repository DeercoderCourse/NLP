...
def parse_parameter(argv):
    ...
    try:
        ...
        print 'please use the format like ex2.py --train=trainfile --ngram-length=2 --model=modelfile --smooth=0.1'
        print 'or ex2.py --test=testfile --ngram-length=2 --model=modelfile'
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print 'ex2.py --train [training-data-file] [ngram-length: int] --model [model-file]'
        elif opt == "--train":
            trainfile = arg
        elif opt == "--test":
            testfile = arg
